...
def backtest(df: pd.DataFrame, min_points: int, min_diff_points: int, rounding_nb: float, take_profit: float,
    ...
    prices_groups = {'supports': dict(), 'resistances': dict()}
    ...
    for i in range(len(highs)):
        ...
        for side in ['resistances', 'supports']:
            ...
            if row['rounded_' + h_l][i] in prices_groups[side]:

                grp = prices_groups[side][row['rounded_' + h_l][i]]
                ...
                if broken_in_last < 3 and grp['last'] is None or idx >= grp['last'] + min_diff_points * candle_length:
                    ...
                    if len(grp['prices']) >= min_points:
                        ...
                        levels[side].append([(grp['start_time'], extreme_price), (idx, extreme_price)])
                ...
                if broken_in_last < 3:
                    prices_groups[side][row['rounded_' + h_l][i]] = {'prices': [row[h_l][i]], 'start_time': idx, 'last': idx}

            # check whether price groups are valid or not
    ...
    # mpf.plot(tdf, type='candle', tight_layout=True, datetime_format='%b %d')
    # plt.show()
